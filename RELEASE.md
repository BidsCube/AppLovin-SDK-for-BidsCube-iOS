...
- Relaxed `AppLovinSDK` CocoaPods constraint to `~> 13.2` so apps that pin **AppLovinSDK 13.2.x** (typical on **iOS 14**) no longer conflict with Bidscube pods.
- SwiftPM AppLovin dependency uses `from: "13.2.0"` instead of an exact 13.6.2 pin.
- No functional SDK API changes vs **1.1.5**.
...
2. **Verify spec layout** — `./scripts/sync-cocoapods-spec.sh` and confirm:
   - `BidscubeSDKAppLovin/1.1.6/BidscubeSDKAppLovin.podspec`
   - `BidscubeSDKAppLovinLegacy/1.1.6/BidscubeSDKAppLovinLegacy.podspec`
...
5. **Commit** on `main`, including both `1.1.6` spec folders.
6. **Tag and push** — `git tag v1.1.6 && git push origin main && git push origin v1.1.6`
...
Update the pod version to **1.1.6** and run `pod install --repo-update`. No API migration required.
...
- Notify integrators: same dual-pod layout, **1.1.6** for both modern and legacy.
