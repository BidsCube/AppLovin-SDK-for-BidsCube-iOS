#!/bin/bash
# Build, install, and launch the legacy test app on an iOS Simulator.
set -euo pipefail
...
SCHEME="LegacySmoke"
BUNDLE_ID="com.bidscube.legacysmoke"
DERIVED_DATA="${DERIVED_DATA:-/tmp/legacy-test-app-dd}"
...
APP_PATH="$DERIVED_DATA/Build/Products/Debug-iphonesimulator/LegacySmoke.app"
if [ ! -d "$APP_PATH" ]; then
...
echo "Legacy test app launched on simulator $SIMULATOR_UDID"
echo "Configure MAX ad units in LegacySmoke/Info.plist or export MAX_* before running."
