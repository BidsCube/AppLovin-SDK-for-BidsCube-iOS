...
A MAX test app lives in [`testApp/`](testApp/). See [`testApp/README.md`](testApp/README.md) for setup.

```bash
pod install
open bidscubeSdk.xcworkspace   # generated by CocoaPods
```

In Xcode, run scheme **testApp-ios** (iOS 15+). Set `APPLOVIN_SDK_KEY` and `MAX_*_AD_UNIT_ID` via scheme environment variables or `testApp-Info.plist`.

Optional SSP override for QA:
...
2. **Configure MAX Dashboard** — [Integrating custom SDK networks](https://support.axon.ai/en/max/mediated-network-guides/integrating-custom-sdk-networks/):
   ...
   - **iOS Adapter Class Name:** `ALBidscubeMediationAdapter`
   - Enable **Bidscube** on each MAX ad unit (banner, MREC, interstitial, rewarded, native).
...
6. **Load ads** with standard MAX APIs (`MAAdView`, `MAInterstitialAd`, `MARewardedAd`, `MANativeAdLoader`, etc.).
